...
if (prefixedPjsonVersion !== gitTag) {
  console.error(
    `Version field in package.json, ${pjsonVersion}, does not match the current git tag, ${gitTag}. See docs/releasing.md`
  )
  process.exit(1)
}

if (semver.valid(gitTag) === false) {
  console.error(
    `Git tag ${gitTag} is not a valid semver version. See docs/releasing.md`
  )
  process.exit(1)
}
